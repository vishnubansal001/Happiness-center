...
export default function Events() {
  ...
  return (
    ...
    <div className="w-screen p-1 h-full my-6"><h1 className='font-monts font-bold flex items-center justify-start ml-4 sm:ml-6 md:ml-12 gap-2 text-xl md:text-4xl p-1 uppercase text-black'>Upcoming<span className="text-[#EF1C22]"> Events</span></h1>
    {loading ? <Spinner/> : events && events.length > 0 ? events?.map((event,index) => {
          return (
            <div key={index} className="flex flex-col md:flex-row items-center justify-center gap-1 h-full w-full">
              ...
              <div className="flex flex-col items-center md:items-start md:gap-6 w-full h-full md:w-[54%] justify-center mb-4  md:my-8">
                ...
                <Link  to={`/events/${event.id}/registration`}> <div className="flex items-center justify-center bg-[#FB393F] w-[12rem] h-[2.6rem] p-2 shadow-[4px_4px_rgba(0,0,0,1)]">
    ...
    <section className="flex items-center justify-center gap-2 flex-col">
    <div className="w-screen p-1 h-full my-6"><h1 className='font-monts font-bold flex items-center justify-start ml-4 sm:ml-6 md:ml-12 gap-2 text-xl md:text-4xl p-1 uppercase text-black'>Past<span className="text-[#EF1C22]"> Events</span></h1>
    <div className="flex flex-row items-center justify-center w-full py-8 h-full">
        <div className="flex items-center justify-center w-screen p-2">
          ...
          >
            {loading ? <Spinner/> : images && images.length > 0 ? (images?.map((image,index) => {
              return (
