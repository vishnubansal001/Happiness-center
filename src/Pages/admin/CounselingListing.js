...
export default function CounselingListing() {
  ...
  useEffect(() => {
    async function fetchListings() {
      try {
        const registrationsRef = collection(db, "counseling");
        const q = query(
          registrationsRef,
  ...
  async function onFetchMoreListing(){
   ...
   async function downloadText () {
    ...
    try {
      const downloadRef = collection(db, "counseling");
      const q = query(
        downloadRef,
        orderBy("timestamp", "desc"),
      );
