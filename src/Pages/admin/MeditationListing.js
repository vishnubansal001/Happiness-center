...
export default function MeditationListing() {
  ...
  useEffect(() => {
    async function fetchListings() {
      try {
        const registrationsRef = collection(db, "meditation");
        const q = query(
          registrationsRef,
  ...
  async function onFetchMoreListing(){
   ...
   async function downloadText () {
    ...
    try {
      const downloadRef = collection(db, "meditation");
      const q = query(
        downloadRef,
        orderBy("timestamp", "desc"),
      );
