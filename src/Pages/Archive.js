...
export default function Archive() {
  ...
  useEffect(() => {
    async function fetchListings() {
      try {
        ...
        const q = query(archiveRef, orderBy("timestamp", "desc"), limit(12));
  ...
  return (
    <section className="w-screen h-full flex items-center justify-center flex-col">
      ...
      <div
        className="max-w-[90rem] w-full flex-col flex items-center justify-center  p-2"
        id="gallery"
      >
        {loading ? (
          <Spinner />
          ...
          <>
            <div className="w-full mt-4 grid lg:grid-cols-3 xl:grid-cols-4 sm:grid-cols-2 grid-cols-1 items-center justify-center gap-2 p-2">
              {images.map((image) => (
                <div
                  key={image.id}
                  className="flex grid-cols-1 p-2 items-center justify-center bg-white"
                >
                  <img
            ...
            {lastFetchListing && (
              <div className="flex justify-center w-full items-center flex-row mt-3 mb-3">
                <button
                  ...
                  className="bg-[#1758E8] p-3 text-gray-100 border border-gray-300 hover:border-slate-600 rounded transition duration-150 ease-in-out text-xs"
                >
                  Load More
                </button>
              </div>
